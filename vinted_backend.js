            ...
            const listingData = await page.evaluate(() => {
                ...
                allImages.forEach(img => {
                    ...
                    if (src && (
                        ...
                        let highResSrc = src;
                        if (src.includes('_')) {
                            highResSrc = src.replace(/_\d+x\d+/, '');
                        }
                        // Remove trailing colon if present
                        highResSrc = highResSrc.replace(/:$/, '');
                ...
                return {
                    ...
                    images: Array.from(imageUrls).slice(0, 20) // Limit to 20 images
                };
            });
    ...
    try {
        ...
        const downloadPromises = listingData.images.map(async (imageUrl, index) => {
            try {
                ...
                const filename = path.join(sessionDir, `image_${index + 1}.${extension}`);
...
app.get('/api/download/:sessionId', async (req, res) => {
    try {
        ...
        const zipFilename = path.join(tempDir, sessionId, 'images.zip');
        ...
        // Send the ZIP file
        res.download(zipFilename, 'vinted_images.zip', (error) => {
            if (error) {
